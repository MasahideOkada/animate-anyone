...
def parse_args(input_args=None):
    parser = argparse.ArgumentParser(description="Animate Anyone stage2 training script.")
    parser.add_argument(
        "--pretrained_dir",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "--mm_dir",
        type=str,
        default="motion-modules",
        required=False,
        help="sub folder of pretrained AnimateDiff's motion modules in the `pretrained_checkpoint_dir`",
    )
    parser.add_argument(
        "--stage1_dir",
        type=str,
        default=None,
        required=True,
        help="a folder of pretrained 1st stage Animate Anyone models in the `pretrained_checkpoint_dir`",
    )
    parser.add_argument(
        "--revision",
        type=str,
    ...
    parser.add_argument(
        "--num_frames", type=int, default=24, help="Number of frames to sample from a video"
    )
...
def main(args):
    ...
    # Loading stage 1 denoising unet, reference net, and pose guider
    motion_module_path = os.path.join(args.pretrained_dir, args.mm_dir)
    logger.info(f"Loading weights from {args.stage1_dir} to initialize UNet, ReferenceNet, and PoseGuider")
    unet = UNet3DConditionModel.from_pretrained_1st_stage_and_animate_diff(
        args.stage1_dir, motion_module_path=motion_module_path, subfolder="unet", use_safetensors=True
    )
    refnet = UNet2DConditionModel.from_pretrained(
        args.stage1_dir, subfolder="referencenet", use_safetensors=True
    )
    pose_guider = PoseGuider.from_pretrained(
        args.stage1_dir, subfolder="pose_guider", use_safetensors=True
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                bsz = latents.shape[0] // num_frames
