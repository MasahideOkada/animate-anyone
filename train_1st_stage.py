...
---
    """
    model_card = f"""
# animate-anyone-{repo_id}

These are animate-anyone weights trained on {base_model} with new type of conditioning.
{img_str}
"""
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "A csv file to specify data video files"
        ),
    )
    parser.add_argument(
...
def main(args):
    ...
    noise_scheduler = DDPMScheduler.from_pretrained(sd_dir, subfolder="scheduler")
    vae = AutoencoderKL.from_pretrained(
        sd_dir, subfolder="vae", revision=args.revision, variant=args.variant
    )

    # Make denoising unet, reference net, and pose guider
    logger.info(f"Loading weights from {args.sd_dir} to initialize UNet and ReferenceNet")
    unet = UNet3DConditionModel.from_pretrained_sd(
        sd_dir, subfolder="unet", use_safetensors=True
    )
    refnet = UNet2DConditionModel.from_pretrained_sd(
        sd_dir, subfolder="unet", use_safetensors=True
    )
    logger.info(f"initializing PoseGuider")
    pose_guider = PoseGuider(unet.config.in_channels)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                ...
                assert len(models) == 3 and len(weights) == 3
                while len(weights) > 0:
                    sub_dir = "unet"
                    if isinstance(model, UNet2DConditionModel):
                        sub_dir = "referencenet"
                    elif isinstance(model, PoseGuider):
                        sub_dir = "pose_guider"
    ...
    if args.gradient_checkpointing:
        ...
        refnet.enable_gradient_checkpointing()
    ...
    if accelerator.unwrap_model(refnet).dtype != torch.float32:
        raise ValueError(
            f"ReferenceNet loaded as datatype {accelerator.unwrap_model(refnet).dtype}. {low_precision_error_string}"
        )    

    if accelerator.unwrap_model(pose_guider).dtype != torch.float32:
        raise ValueError(
            f"PoseGuider loaded as datatype {accelerator.unwrap_model(pose_guider).dtype}. {low_precision_error_string}"
        )    
    ...
    # Optimizer creation
    params_to_optimize = (
        list(unet.parameters()) + list(refnet.parameters()) + list(pose_guider.parameters())
    )
    ...
    unet, refnet, pose_guider, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        unet, refnet, pose_guider, optimizer, train_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with (
                ...
                # Encode pose sequence and add it to latents
                pose_embeds = pose_guider(
                    batch["pose_pixel_values"].to(latents.device, dtype=weight_dtype)
                )
                noisy_latents = noisy_latents + pose_embeds

                # Get CLIP embedding of reference image for cross attention
                ref_pixel_values = batch["ref_pixel_values"].to(latents.device, dtype=weight_dtype)
                ref_clip_values = clip_processor(
                    ref_pixel_values, do_rescale=False, return_tensors="pt"
                )["pixel_values"].to(latents.device, dtype=weight_dtype)
    ...
    if accelerator.is_main_process:
        ...
        refnet = accelerator.unwrap_model(refnet)
        refnet.save_pretrained(os.path.join(args.output_dir, "referencenet"))
